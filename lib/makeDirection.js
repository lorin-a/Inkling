...
export async function buildDirectionBoard({ pins = [], colours = [], reflectionYes = "", reflectionNo = "" }) {
  ...
  // 2 — the colours you gathered, as a swatch strip that wraps.
  const SW = 84;
  const valid = colours.filter((h) => /^#[0-9a-fA-F]{6}$/.test(h)).map((h) => h.toLowerCase());
  if (valid.length) {
    const perRow = Math.max(1, Math.floor((boardW + GAP) / (SW + GAP)));
    valid.forEach((hex, i) => {
      const col = i % perRow;
  ...
  const name = yes
    ? truncate(yes, 40)
    : valid[0]
    ? `${colorName(valid[0]).name} direction`
    : "New direction";
