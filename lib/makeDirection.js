 ...
 * the references you kept in an Imagery zone, and (lazily, once you keep a face on /type)
 * a Type zone. Your own words ride on the Direction card, not the canvas. This is the
 * relief moment (overwhelm → tangible): you finish reacting and arrive to find it already
 * sorted, not a blank canvas you have to fill again.
 *
 ...
 */
async function composeMaster(existing, { pins = [], colours = [] }) {
  ...
  // No empty Type zone here: it would land as a big dead "Type 0" box. The Type zone
  // is created lazily by addTypeToBoard (ensureZone) when you actually keep a face.
  ...
  // Your words are NOT placed on the canvas (they used to land as a text block at the
  // top, clipped under the toolbar and duplicating the Direction card). They now live on
  // the Direction artifact itself — written to its per-board "why" by createDirection.
...
export async function createDirection(input) {
  ...
  // Your words become the Direction's "why" (the same per-board key the Direction card
  // reads on the board and on Compose) — so they travel with the artifact instead of
  // sitting on the canvas as a clipped note. Don't clobber a why you've already written.
  const why = (input.reflectionYes || "").trim();
  if (why) {
    try {
      const key = `moodbuilder.direction.why.${master.id}`;
      if (!localStorage.getItem(key)) localStorage.setItem(key, why);
    } catch { /* storage off — the words are still in the recognize session */ }
  }
