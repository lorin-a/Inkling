...
export function buildProfile(reactions = []) {
  ...
  for (const r of reactions) {
    ...
    if (w > 0) {
      ...
      // A few more colours per pin than the steering uses, so the curation pool
      // she picks from is rich — secondary tones, not just the dominant four.
      for (const hex of pal.slice(0, 6)) {
