...
export async function listBoards({ projectId }) {
  ...
  const rows = await sql`
    SELECT id, name, blocks, sections, comments, background, created_at, updated_at
...
export async function createBoard({ projectId, name }) {
  ...
  const rows = await sql`
    ...
    RETURNING id, name, blocks, sections, comments, background, created_at, updated_at
  `;
...
export async function saveBoard({ projectId, id, patch }) {
  ...
  if (patch && Object.prototype.hasOwnProperty.call(patch, "sections")) {
    sets.push(`sections = $${sets.length + 1}`);
    values.push(JSON.stringify(Array.isArray(patch.sections) ? patch.sections : []));
  }
  if (patch && Object.prototype.hasOwnProperty.call(patch, "comments")) {
    sets.push(`comments = $${sets.length + 1}`);
    values.push(JSON.stringify(Array.isArray(patch.comments) ? patch.comments : []));
  }
  if (patch && Object.prototype.hasOwnProperty.call(patch, "background")) {
    sets.push(`background = $${sets.length + 1}`);
    values.push(patch.background || null);
  }
