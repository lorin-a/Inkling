...
export async function boardForToken(t) {
  ...
  if (!me[0]) return null;
  const member = me[0];
  const [boards, members, votes, words, comments] = await Promise.all([
    ...
    sql`SELECT c.id, c.member_id, c.card_id, c.text, c.created_at, m.name FROM studio_comments c JOIN studio_members m ON m.id = c.member_id WHERE c.board_id = ${member.board_id} ORDER BY c.created_at ASC`,
  ]);
  const board = boards[0];
  if (!board) return null;
  return shape({ board, member, members, votes, words, comments });
}
 ...
 */
function shape({ board, member, members, votes, words, comments = [] }) {
  ...
  // First words reveal at the word level: once you have written one of your
  // own, you see everyone's. The votes keep their stricter rule.
  const allWords = revealOpen || myWords.length > 0 ? words.map(wordOut) : [];

  const threads = {};
  for (const c of comments) {
    (threads[c.card_id] ||= []).push({ id: c.id, cardId: c.card_id, memberId: c.member_id, by: c.name, mine: c.member_id === member.id, text: c.text, at: toIso(c.created_at) });
  }
...
export async function addMember({ boardId, name }) {
  const have = await sql`SELECT COUNT(*)::int AS n FROM studio_members WHERE board_id = ${boardId} AND role = 'member'`;
  if ((have[0]?.n || 0) >= MAX_COLLABORATORS) throw new Error(`Up to ${MAX_COLLABORATORS} collaborators`);
  const t = token();
  const rows = await sql`
...
export async function renameMember({ memberId, name, boardId = null }) {
  if (boardId) await sql`UPDATE studio_members SET name = ${name} WHERE id = ${memberId} AND board_id = ${boardId}`;
  else await sql`UPDATE studio_members SET name = ${name} WHERE id = ${memberId}`;
}

export async function removeMember({ memberId, boardId }) {
  await sql`DELETE FROM studio_members WHERE id = ${memberId} AND board_id = ${boardId} AND role = 'member'`;
}

export async function addComment({ boardId, memberId, cardId, text }) {
  const rows = await sql`
    INSERT INTO studio_comments (board_id, member_id, card_id, text)
    VALUES (${boardId}, ${memberId}, ${cardId}, ${text})
    RETURNING id, card_id, member_id, text, created_at
  `;
  const r = rows[0];
  const who = await sql`SELECT name FROM studio_members WHERE id = ${memberId}`;
  return { id: r.id, cardId: r.card_id, memberId: r.member_id, by: who[0]?.name || "", mine: true, text: r.text, at: toIso(r.created_at) };
}

export async function deleteComment({ boardId, memberId, id }) {
  await sql`DELETE FROM studio_comments WHERE id = ${id} AND board_id = ${boardId} AND member_id = ${memberId}`;
}
