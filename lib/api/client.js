...
function routeLocal(path, opts) {
  ...
  switch (`${method} ${url}`) {
    ...
    case "GET /api/marks":
      // Marks need Blob storage (deferred to authed-only). Signed-out
      // visitors have none — return empty rather than the file-based set.
      return jsonResponse({ marks: [] });

    case "GET /api/textures":
      // Textures are file uploads too — empty until Blob storage ships.
