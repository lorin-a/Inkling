...
/** sRGB → OKLab (the Cartesian form, for perceptual distance). */
...
export function oklabFromHex(hex) {
  return rgbToOklab(hexToRgb(hex));
}

/**
 * Perceptual distance between two colors — Euclidean in OKLab, which is
 * close enough to a ΔE for ranking and separation tests. Roughly: < 0.02
 * is barely perceptible, ~0.1 reads as a clearly different color, > 0.25
 * is a strong contrast of hue/lightness. Used both to keep roles from
 * collapsing into near-duplicates and to score how *representative* a
 * candidate is of the user’s pool (local density).
 */
export function deltaEOK(hex1, hex2) {
  const a = oklabFromHex(hex1);
  const b = oklabFromHex(hex2);
...
/**
 * Nudge a color’s OKLCH lightness until it clears `target` contrast against
 * `bg`, keeping hue and chroma fixed so the color keeps its identity. Darkens
 * when bg is the lighter of the two, lightens when bg is darker. Returns the
 * original hex if it already clears, or if no L in range reaches the target
 * (e.g. bg sits mid-luminance and the hue can’t escape in either direction).
 *
 * This is how the light variant keeps a real, on-brand accent: the pool’s
 * chromatic colors are mostly mid-light, so a vivid mid can’t contrast on a
 * pale bg. Rather than fall back to a duller pool color, we deepen the accent
 * in place — same hue, more legible.
 */
export function tintToContrast(hex, bg, target) {
  if (contrastRatio(hex, bg) >= target) return hex;

  const { L, C, h } = oklchFromHex(hex);
  // Push L away from bg’s luminance: darker if bg is lighter, else lighter.
  const goDarker = relativeLuminance(bg) > relativeLuminance(hex);
  const step = 0.02;
  let best = hex;

  for (let i = 1; i <= 50; i++) {
    const nextL = goDarker ? L - step * i : L + step * i;
    if (nextL <= 0 || nextL >= 1) break;
    const candidate = hexFromOklch({ L: nextL, C, h });
    if (contrastRatio(candidate, bg) >= target) return candidate;
    best = candidate;
  }
  // Couldn’t reach target within range — return the closest we got, which is
  // still more legible than the original.
  return best;
}
...
export const THRESHOLDS = {
  ...
  MIN_ROLE_DELTAE: 0.12, // roles must be perceptually distinct, not near-dupes
  TASTE_RADIUS: 0.16, // ΔE radius for counting a color’s pool neighbors
};
