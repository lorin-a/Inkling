...
const KEYS = {
  ...
  firstRun: "moodbuilder.local.firstRunChosen.v1", // user has made the door choice
  mode: "moodbuilder.local.startMode.v1",          // "own" | "sample"
...
// The empty studio — the DEFAULT for a fresh visitor. Your own work is the
// baseline; the sample is opt-in (seedSample). This is what makes "start your
// own" land in a clean project with no sample bleed.
const EMPTY_LIBRARY = { schemaVersion: 1, pins: {}, boards: [], starred: [], starredPalettes: [] };
const EMPTY_PALETTE = { brand: {}, inspiration: { source: [], curated: {} } };
const EMPTY_PROJECT = { name: "Untitled", slug: SAMPLE_SLUG, wordmark: "", period: ".", initial: "", tagline: "", body: "", fonts: {} };
const freshEmptyLibrary = () => structuredClone(EMPTY_LIBRARY);
const freshEmptyPalette = () => structuredClone(EMPTY_PALETTE);
const freshEmptyProject = () => structuredClone(EMPTY_PROJECT);
...
export function ensureSeeded() {
  if (read(KEYS.seeded, false)) {
    // Existing visitor from before the first-run choice shipped: they already
    // have (sample) data, so treat them as having implicitly chosen the sample
    // — don't bounce them to the door.
    if (read(KEYS.firstRun, null) == null) {
      write(KEYS.firstRun, true);
      write(KEYS.mode, "sample");
    }
    ensureSampleMarks(); // one-time backfill for sessions seeded before marks shipped
    return;
  }
  // Brand-new visitor: initialize EMPTY. Their own work is the default; the
  // sample is opt-in via seedSample(). firstRun is written EXPLICITLY false (not
  // left absent) so the door shows AND survives reloads — and so the migration
  // branch above (which only fires when firstRun is absent) never mistakes a
  // fresh, undecided visitor for a pre-change one.
  write(KEYS.project, freshEmptyProject());
  write(KEYS.library, freshEmptyLibrary());
  write(KEYS.palette, freshEmptyPalette());
  write(KEYS.marks, []);
  write(KEYS.marksSeeded, true);
  write(KEYS.activeSlug, SAMPLE_SLUG);
  write(KEYS.seeded, true);
  write(KEYS.firstRun, false);
}

/** Has the visitor made the first-run door choice yet? */
export function hasChosenStart() {
  ensureSeeded();
  return !!read(KEYS.firstRun, false);
}

/** "own" | "sample" | null — which door they walked through. */
export function getStartMode() {
  return read(KEYS.mode, null);
}

/** Start a clean, empty project of your own (named). No sample, ever. */
export function startOwn(name) {
  write(KEYS.project, { ...freshEmptyProject(), name: String(name || "Untitled").slice(0, 80) });
  write(KEYS.library, freshEmptyLibrary());
  write(KEYS.palette, freshEmptyPalette());
  write(KEYS.marks, []);
  write(KEYS.marksSeeded, true);
...
/** Opt into the sample studio (for the guided tour). */
...
function getProjectRaw() {
  ensureSeeded();
  return { ...PROJECT_DEFAULTS, ...read(KEYS.project, freshEmptyProject()) };
}
function getLibraryRaw() {
  ensureSeeded();
  return read(KEYS.library, freshEmptyLibrary());
}
function getPaletteRaw() {
  ensureSeeded();
  const p = read(KEYS.palette, freshEmptyPalette());
  return {
    brand: p.brand || {},
    inspiration: {
...
export function saveBoard(id, patch = {}) {
  ...
  if (Array.isArray(patch.sections)) board.sections = patch.sections;
  if (Array.isArray(patch.comments)) board.comments = patch.comments;
