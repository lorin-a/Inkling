...
/**
 * The pile sized to what is in it. Sixty cards want the whole board; twenty
 * five want a box a third the size, or they read as crumbs on a table.
 */
export function pileBox(n) {
  const w = Math.round(Math.min(PILE.w, Math.max(720, Math.sqrt(Math.max(n, 1)) * 118 * 2.3)));
  const h = Math.round(Math.min(PILE.h, Math.max(520, w * 0.66)));
...
/** The Group board sized to what is on it, with its working field. */
export function groupBox(n) {
  const w = Math.round(Math.min(GROUPS.w, Math.max(900, Math.sqrt(Math.max(n, 1)) * 85 * 3.4)));
  const h = Math.round(Math.min(GROUPS.h, Math.max(640, w * 0.72)));
  const box = { x: GROUPS.x, y: GROUPS.y, w, h };
  // The field starts well below the top edge: every group carries its name
  // above itself, and the first row needs somewhere for that name to live.
  return { box, field: { x: box.x + 30, y: box.y + 160, w: box.w - 60, h: box.h - 200 } };
}
...
export function cardSize(card, board) {
  ...
  if (card.kind === "note") return { w: Math.round(150 * k), h: Math.round(110 * k) };
